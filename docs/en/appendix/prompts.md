...
description: "Ready-to-use prompt templates, copy and use instantly"
---
...
- If requested genre is unfamiliar, explain and suggest similar genres
````
...
- ❌ Avoid stereotyping (all good or all bad)
````
...
### 🎬 Golden Three Chapters (Web Fiction)
...
- ❌ Avoid overly long internal monologues
````
...
- ❌ Avoid exaggerated claims that violate advertising regulations
````
...
- If encountering polysemous words, mark possible translations for user selection
````

---

### ✏️ Paraphrasing for Originality
...
- ❌ Avoid changing core viewpoints
````
...
- If code has syntax errors, point out errors first then explain intent
````
...
- If tech stack is unfamiliar, explain and suggest alternatives
````
...
- If multiple independent issues exist, suggest handling them one by one
````
...
- ❌ Avoid adding information not in original text
```
...
Focus on: Code quality, potential issues, security risks, best practices
```
...
3. Finally use "Test Case Generator" to add tests
```
...
- [Creative Workflow](/en/4-scenarios/writer-workflow) - Content creation tutorial
- [Developer Daily](/en/4-scenarios/coder-daily) - Development scenario tutorial
- [Custom Commands](/en/5-advanced/04-commands) - Custom command configuration
